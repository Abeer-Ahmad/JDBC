...
public class SelectValidator {
	...
	// validates SELECT syntax
	public ArrayList<String> validateSelect(String[] keys) throws SQLException {
		initialize();
    	setKeys(keys);
		if (keys.length < Constants.SELECT_MIN) {
			declareError("SELECT", "Missing Information!");
		} else {
			try {
				int j = 1;
				if (keys[j].equalsIgnoreCase("distinct")) {
					query.add(keys[j]);
					j++;
				}
				int i = j + 1;
				if (!keys[j].equals("*")) {
					query.add(keys[j].toLowerCase()); // toLowerCase()
					i = validateComma(i);
				}
				if (keys[i].equalsIgnoreCase("from")) {
					query.add(Constants.BEGIN, keys[i + 1].toLowerCase());//table to lowercase
					test(i + 2, Constants.SELECT_TABLE, "SELECT");
				} else {
					declareError("SELECT", "\"FROM\" Keyword Is Missing!");
				}
			} catch (Exception e) {
				declareError("SELECT", e.toString());
